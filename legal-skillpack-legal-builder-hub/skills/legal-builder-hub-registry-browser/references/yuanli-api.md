...
站点根 URL 记为 `{BASE}`（profile 默认 `https://yuanli.ailaw.cn`，[法律元力]）。
...
| 能力 | 推荐 API | 对应 skill |
|------|----------|------------|
| 浏览 法律元力 工具包 | `GET /api/toolkits`、`/catalog` | `registry-browser` |
| 安装 zip 包 | `GET /api/toolkits/{id}/download` | `skill-installer` |
| 检查包更新 | `GET /api/toolkits/catalog` | `auto-updater` |
| 浏览法律元力站内 skill | `GET /api/skills?search=` | `registry-browser` |
